    ...
    <>
      <div className={`form-group ${containerStyle}`}>
        ...
        <Listbox value={selected.value} onChange={(value) => {
          ...
          <Listbox.Button className="relative w-full cursor-default rounded bg-white py-2 pl-3 pr-10 text-left !leading-5 focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300">
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
          <Listbox.Options className="absolute mt-1 ml-0 py-1 mb-4 max-h-60 w-full overflow-auto rounded-md bg-white text-base z-10 ring-1 ring-black ring-opacity-5 focus:outline-none">
              {options.map((option, optionIdx) => (
                <Listbox.Option
                key={optionIdx}
                className={({ active }) =>
                  `relative cursor-default select-none py-1 pl-10 pr-4 ${
                    active ? 'bg-primaryColor bg-opacity-50 text-textPrimary' : 'text-textPrimary'
                  }`
                }
                value={option.value}
                >
                  {({ selected }) => (
                    <>
                      ...
                      {selected
                        ? (
                          <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-textPrimary">
